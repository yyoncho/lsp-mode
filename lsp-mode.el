...
(defun lsp--log-time (method id start-time before-send received-time after-parsed-time after-processed-time)
  (lsp-log "
RequestId: %s (%s)
...
"
           ...
           (if after-processed-time
             ...
             "N/A")))
;; (format "%.06f" (float-time (time-subtract time2 time)))
      ...
      (pcase (lsp--get-message-type json-data)
        ('response
         ...
         (-let [(callback _ method start-time before-send) (gethash id (lsp--client-response-handlers client))]
           ...
           (lsp--log-time method id start-time before-send received-time after-parsed-time after-processed-time)))
        ('response-error
         (cl-assert id)
