...
(ert-deftest lsp-test-current-org-mode-content ()
...
(ert-deftest lsp-position-translation-test ()
  (with-current-buffer (find-file-noselect "fixtures/org-mode/demo.org")
    (goto-char (point-min))
...
")
                (should (equal (lsp--buffer-content)
                               (with-current-buffer "*org_demo_file_2.py*"
                                 (buffer-string)))))
            (delete-region (point) (- (point) (length "     return 0;
")))
            (should (equal (lsp--buffer-content)
                           (with-current-buffer "*org_demo_file_2.py*"
                             (buffer-string))))))
