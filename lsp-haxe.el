...
(defcustom lsp-haxe-node-executable "node"
  "Haxe ls is executed via node."
  :group 'lsp-haxe
  :risky t
  :type 'file)
...
(defcustom lsp-haxe-server-path ""
...
(defun lsp-clients--haxe-command ()
  ...
  `(,lsp-haxe-node-executable ,lsp-haxe-server-path))
...
(lsp-register-client
 (make-lsp-client :new-connection (lsp-stdio-connection
                  ...
                  :initialization-options  `((:displayArguments . ["build.hxml"])
                                             (:displayServerConfig . ((:path . "haxe")
                                                                      (:env . ,(ht))
                                                                      (:arguments . [])
                                                                      (:print . ((:completion . :json-false)
                                                                                 (:reusing . :json-false)))))
                                             (:sendMethodResults . t))
                  :priority -1
                  :server-id 'haxe
